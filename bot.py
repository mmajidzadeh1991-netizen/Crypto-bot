...
# لیست ارزها متصل به موتور استاندارد TradingView (با مرجع دیتای صرافی بایننس برای تیکرهای USDT)
TOP_COINS = [
...
# بررسی سشن‌های معاملاتی (Killzones) برای جلوگیری از ترید در ساعات پرنویز بازار
def is_optimal_trading_session():
    current_hour = datetime.utcnow().hour
    # سشن‌های لندن و نیویورک (بهترین زمان نقدینگی)
    if 7 <= current_hour <= 21:
        return True
    return False

# ارتباط با هوش مصنوعی Groq با پرامپت حرفه‌ای ICT/SMC و ژورنال خودآموز
def ask_groq_ai_institutional(prompt_text):
    ...
    payload = {
        "model": "llama-3.3-70b-versatile",  # آپدیت شده به مدل جدید و پایدار
        "messages": [
    ...
    try:
        ...
        if "choices" in res_data:
            ...
            return f"خطای سرور گروق: {res_data}"
    except Exception as e:
        print(f"Groq API Connection Exception: {e}")
        return f"خطای ارتباط با هوش مصنوعی: {str(e)}"

# تحلیل چندتایم‌فریمه بازار با استفاده از موتور رایگان و بدون تحریم TradingView
def analyze_institutional_market(coin):
    try:
        ...
        print(f"TradingView Technical Analysis Error for {coin['symbol']}: {e}")
        return None

# اسکنر خودکار هوشمند متصل به تریدینگ‌ویو با فیلتر سشن‌های معاملاتی
def institutional_trader_scanner():
    print("🏛️ ربات تریدر نهادی (متصل به موتور قدرتمند تریدینگ‌ویو) فعال شد...")
    while True:
        try:
            if is_optimal_trading_session():
                for coin in TOP_COINS:
                    print(f"در حال ارزیابی نهادی {coin['symbol']} از طریق تریدینگ‌ویو در سشن فعال...")
                    ...
                    if signal and ("جهت پوزیشن" in signal or "Long" in signal or "Short" in signal):
                        ...
                        full_msg = f"🏛️ **سیگنال نهادی و پیشرفته (تریدینگ‌ویو + سشن فعال)** 🏛️\n\n{signal}"
                        send_telegram_message(DEFAULT_CHAT_ID, full_msg)
                        time.sleep(14400) # ۴ ساعت فاصله پس از ارسال سیگنال موفق
...
def telegram_webhook():
    ...
    try:
        if "callback_query" in update:
            ...
            send_telegram_message(chat_id, f"⏳ در حال تحلیل نهادی چندتایم‌فریمه از تریدینگ‌ویو برای {data}...")
            ...
            if ai_response:
                ...
                send_telegram_message(chat_id, "⚪ پاسخ خالی از هوش مصنوعی دریافت شد. لطفاً کلید API را بررسی کنید.")
            return "ok", 200
        ...
        if "message" in update:
            ...
            if text.startswith("/start") or text.startswith("/help"):
                welcome_msg = (
                    "🤖 **سیستم تریدر نهادی هوشمند (موتور تریدینگ‌ویو + سشن‌های طلایی)**\n\n"
                    "سلام! ربات شما مجهز به تحلیل مولتی‌تایم‌فریم (گرفته شده از تریدینگ‌ویو)، فیلتر سشن‌های نقدینگی و تعیین حد ضررهای ساختاری است.\n"
                    "از منوی زیر می‌توانید برای بررسی دستی ارزها استفاده کنید:"
                )
                send_telegram_message(chat_id, welcome_msg, reply_markup=keyboard)
            else:
                send_telegram_message(chat_id, "⏳ در حال بررسی عمیق بازار از طریق موتور تریدینگ‌ویو...")
...
def index():
    return "Institutional Trading Bot via TradingView Engine is running!", 200
